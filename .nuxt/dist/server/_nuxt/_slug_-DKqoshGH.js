...
import { defineComponent, toRef, computed, mergeProps, useSSRContext, ref, watch, onUnmounted, unref, withCtx, openBlock, createBlock, createVNode, createTextVNode, toDisplayString, resolveComponent, watchSyncEffect } from "vue";
...
import { _ as _sfc_main$e } from "./TheBreadcrumbs-CyXOlUWJ.js";
import { T as TheContentBlock } from "./TheContentBlock-SWn52iXd.js";
import { _ as _sfc_main$b } from "./TheTextContent-Bd9flYOl.js";
import { _ as _sfc_main$c } from "./TheButton-CLFIQZye.js";
import { _ as _sfc_main$d } from "./TheModal-DtMcuqX0.js";
  ...
  setup(__props) {
    ...
    watch(() => authStore.user, (newValue) => {
      if (list.value.toArray().length === 0) {
        appendData([{
          first_name: newValue == null ? void 0 : newValue.first_name,
          last_name: newValue == null ? void 0 : newValue.last_name,
          photo_url: newValue == null ? void 0 : newValue.photo,
          id: newValue == null ? void 0 : newValue.id
        }]);
      }
    });
    onUnmounted(() => {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      }, _attrs))} data-v-7e7f5d4f>`);
      ssrRenderList(unref(documentStore).userList, (userGroup, groupIndex) => {
        var _a;
        _push(`<div class="px-2 h-full z-20 relative flex items-center transition-all" data-v-7e7f5d4f><div class="transition-all" data-v-7e7f5d4f></div><div class="${ssrRenderClass([{ "border border-dashed border-blueSemiLight h-20 rounded-lg": userGroup.length === 0 || userGroup === null }, "w-[220px] gap-y-6 flex items-center flex-col justify-center relative"])}" data-v-7e7f5d4f>`);
        if (userGroup.length === 0) {
          _push(`<p class="absolute text-blueSemiLight tracking-widest text-sm w-full h-full flex items-center justify-center rounded-lg transition-all duration-200" data-v-7e7f5d4f>${ssrInterpolate(_ctx.$t("Область для добавления"))}</p>`);
        } else {
          _push(`<!---->`);
        }
        if (groupIndex !== 0 && !((_a = userGroup[0]) == null ? void 0 : _a.id)) {
          _push(`<div class="absolute top-0" data-v-7e7f5d4f><button class="rounded-lg border border-gray-500 dark:border-white absolute px-2.5 pt-1 -top-10 right-10 text-green-400 font-bold" data-v-7e7f5d4f> + </button><button class="rounded-lg border border-gray-500 dark:border-white absolute px-2.5 pt-1 -top-10 left-10 text-red-400 font-bold" data-v-7e7f5d4f> x </button></div>`);
        } else {
          _push(`<!---->`);
        }
        if (unref(documentStore).showContext === groupIndex) {
          _push(`<div class="absolute rounded-lg dark:bg-gray-700 bg-gray-200 w-[220px] top-10 z-[5] p-2" data-v-7e7f5d4f><input type="text" class="pl-2 dark:bg-gray-600 rounded-lg outline-none w-full mb-4"${ssrRenderAttr("placeholder", _ctx.$t("Поиск..."))} data-v-7e7f5d4f><div class="overflow-auto max-h-[150px]" data-v-7e7f5d4f><!--[-->`);
          ssrRenderList(unref(company).get_all_department.results, (department) => {
            _push(`<div data-v-7e7f5d4f><button class="flex items-center justify-between gap-x-2 w-full break-words break-all" data-v-7e7f5d4f><span data-v-7e7f5d4f>${ssrInterpolate(department.name)}</span><div class="${ssrRenderClass({ "rotate-180": unref(documentStore).showContextDepartment === department.id })}" data-v-7e7f5d4f><svg class="dark:fill-white fill-black" width="20px" height="20px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" data-v-7e7f5d4f><path fill-rule="evenodd" clip-rule="evenodd" d="M6.46967 8.96967C6.76256 8.67678 7.23744 8.67678 7.53033 8.96967L12 13.4393L16.4697 8.96967C16.7626 8.67678 17.2374 8.67678 17.5303 8.96967C17.8232 9.26256 17.8232 9.73744 17.5303 10.0303L12.5303 15.0303C12.3897 15.171 12.1989 15.25 12 15.25C11.8011 15.25 11.6103 15.171 11.4697 15.0303L6.46967 10.0303C6.17678 9.73744 6.17678 9.26256 6.46967 8.96967Z" data-v-7e7f5d4f></path></svg></div></button>`);
            if (unref(documentStore).showContextDepartment === department.id) {
              _push(`<div data-v-7e7f5d4f><!--[-->`);
              ssrRenderList(department.participants, (user) => {
                ...
                if (department.participants && unref(documentStore).showUsersNode.every((e) => e.id !== user.id) && user.id !== unref(authStore).user.id && userGroup.every((e) => e.priority === user.priority)) {
                  _push(`<button class="px-2 py-1 w-full flex items-center text-start hover:bg-semiCyan rounded-lg transition-all" data-v-7e7f5d4f><span class="" data-v-7e7f5d4f>${ssrInterpolate(user.first_name + " " + user.last_name)}</span></button>`);
                } else {
                  _push(`<!---->`);
                }
                _push(`</div>`);
        ...
        ssrRenderList(userGroup, (user, userIndex) => {
          _push(`<div class="h-24 w-[220px] border border-gray-500 dark:border-white px-2 rounded-lg text-center flex items-center justify-center mx-auto relative" data-v-7e7f5d4f><div class="flex items-center gap-x-2" data-v-7e7f5d4f>`);
          if (user.photo) {
            _push(`<div class="max-w-[50px]" data-v-7e7f5d4f><img class="w-full rounded-full"${ssrRenderAttr("src", user.photo)} alt="" data-v-7e7f5d4f></div>`);
          } else {
            _push(`<!---->`);
          }
          _push(`<div class="break-words" data-v-7e7f5d4f><p class="text-sm w-full" data-v-7e7f5d4f>${ssrInterpolate(user.first_name + " " + user.last_name)}</p></div></div>`);
          if (user.id !== unref(authStore).user.id && groupIndex !== 0) {
            _push(`<button class="absolute px-2.5 pt-1 top-0 right-0 border-l border-b border-gray-500 dark:border-white rounded rounded-tl-none rounded-br-none text-red-400 font-bold" data-v-7e7f5d4f> X </button>`);
          } else {
            _push(`<!---->`);
          }
          _push(`</div>`);
      ...
      _push(`<button class="text-2xl px-4 py-2 ml-4 rounded-lg border border-blueSemiLight text-blueDarkSemiLight cursor-pointer hover:scale-95 transition-all" data-v-7e7f5d4f> + </button></div>`);
    };
  }
});
...
const DocumentItemInfoChainScheduleItem = /* @__PURE__ */ _export_sfc(_sfc_main$9, [["__scopeId", "data-v-7e7f5d4f"]]);
...
//# sourceMappingURL=_slug_-DKqoshGH.js.map
