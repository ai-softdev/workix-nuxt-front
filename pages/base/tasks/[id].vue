...
<template>
  <div class="overflow-hidden">
    ...
    <p
        class="dark:text-white text-3xl font-bold my-8"
    >
      {{ $t('Описание задания') }}
    </p>
    <div class="flex w-full gap-4 max-[1200px]:flex-col">
      <div class="w-10/12 max-lg:w-full flex flex-col gap-4 max-[1200px]:order-2 max-[1200px]:w-full">
        <div class="p-6 rounded-3xl bg-white shadow-objectDescription">
          <div class="mb-8">
            <p class="font-bold mb-4">
              {{ $t('Описание') }}
            </p>
            <div class="w-full border p-4 bg-whiteSmoke rounded-3xl min-h-[200px]">
              {{ currentTask.task.description }}
            </div>
          </div>
          <div class="mb-8">
            <p class="font-bold mb-4">
              {{ $t('Прикреплённые файлы') }}
            </p>
            <div
                class="w-full flex border border-dashed p-4 rounded-3xl"
            >
              <a
                  v-for="files in currentTask.task?.task_files"
                  class="flex items-center gap-2 bg-whiteSmoke p-3 rounded-2xl hover:shadow"
                  :href="files.file" :download="files.file_name"
                  :title="files.file_name"
              >
                <div class="w-9 h-9 rounded-lg bg-white shadow flex items-center justify-center">
              ...
              <div class="w-full text-center mb-4 min-h-[100px]"
                   v-if="currentTask.task.task_files?.length === 0">
                <p>{{ $t('Файлы отсутствуют') }}</p>
              </div>
            </div>
          </div>
<!--          <div>-->
<!--            <p class="font-bold mb-4">-->
<!--              {{ $t('Прикреплённые файлы исполнителей') }}-->
<!--            </p>-->
<!--            <form-->
<!--                enctype="multipart/form-data"-->
<!--                @submit.prevent="currentTask.change_taskStatus({performer_files: files, task_status: 'make-watching', id: currentTask.task.id})"-->
<!--                class="border border-dashed p-4 rounded-3xl"-->
<!--            >-->
...
<!--              </div>-->
<!--              <div-->
<!--                  class="w-full text-center text-mediumGray text-md flex items-center justify-center min-h-[100px]"-->
<!--                  v-if="currentTask.task.performer_files_urls?.length === 0"-->
<!--              >-->
<!--                <p>{{ $t('Файлы отсутствуют') }}</p>-->
<!--              </div>-->
<!--              <div-->
...
<!--          </div>-->
        ...
        <div class="flex items-center justify-between max-[756px]:flex-col max-[756px]:gap-4">
          <p class="text-md text-mediumGray">
            {{ $t('Последние изменения') + ' ' + new Date(currentTask.task.updated_at).toLocaleDateString() + ', ' + new Date(currentTask.task.updated_at).toLocaleTimeString() }}
          </p>
          <button
              class="flex items-center gap-3 rounded-full border py-2.5 px-10 font-bold text-redOrange hover:text-white hover:bg-redOrange transition-all ease-in-out duration-300"
              @click="showDeleteTask = true"
          >
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path
                  d="M15.5556 6.33388V5.62276C15.5556 4.62712 15.5556 4.12929 15.3618 3.749C15.1914 3.41449 14.9194 3.14252 14.5849 2.97209C14.2046 2.77832 13.7068 2.77832 12.7111 2.77832H11.2889C10.2932 2.77832 9.79541 2.77832 9.41513 2.97209C9.08061 3.14252 8.80865 3.41449 8.63821 3.749C8.44444 4.12929 8.44444 4.62712 8.44444 5.62276V6.33388M10.2222 11.2228V15.6672M13.7778 11.2228V15.6672M4 6.33388H20M18.2222 6.33388V16.2894C18.2222 17.7829 18.2222 18.5296 17.9316 19.1001C17.6759 19.6019 17.268 20.0098 16.7662 20.2654C16.1957 20.5561 15.4491 20.5561 13.9556 20.5561H10.0444C8.55097 20.5561 7.80423 20.5561 7.2338 20.2654C6.73204 20.0098 6.32409 19.6019 6.06843 19.1001C5.77778 18.5296 5.77778 17.7829 5.77778 16.2894V6.33388"
      ...
      <div class="w-4/12 h-fit flex flex-col gap-5 max-[1200px]:w-full">
        <div class="p-6 rounded-3xl bg-white shadow-objectDescription">
          <div class="border rounded-full mx-auto px-3 py-2 w-fit flex items-center gap-3 mb-6">
            <img src="/icons/author.svg" alt="author">
            <h2 class="dark:text-white">{{ $t('Автор задания') }}</h2>
          </div>
          <div class="bg-whiteSmoke py-2 px-4 rounded-full w-fit mx-auto">
            {{ currentTask.task?.author?.first_name + ' ' + currentTask.task?.author?.last_name }}
          </div>
        </div>
        <div class="p-6 rounded-3xl bg-white shadow-objectDescription">
          <div class="border rounded-full mx-auto px-3 py-2 w-fit flex items-center gap-3 mb-6">
            <img src="/icons/ispolniteli.svg" alt="ispolniteli">
            <h2 class="dark:text-white">{{ $t('Исполнители') }}</h2>
          </div>
          <div class="mt-2">
            <p class="text-center font-medium">{{ $t('Главный исполнитель') }}</p>
            <NuxtLink
                :to="`/base/users/${currentTask.task.main_performer?.id}`"
                class="flex mt-2 items-center gap-2 bg-whiteSmoke rounded-3xl p-2 max-[1200px]:w-fit max-[1200px]:mx-auto"
            >
              <div class="w-8 h-8 rounded-full shadow flex items-center justify-center bg-white">
                <img
                    class="w-4 h-4 rounded-full"
                    :src="currentTask.task.main_performer?.photo"
                    alt="logo"
                >
              </div>
              <p>
          ...
          <div class="mt-4">
            <p class="text-center font-medium">{{ $t('Список исполнителей') }}</p>
            <div
            ...
            <div
              ...
              <div
                  class="flex relative items-center p-2 gap-x-6 bg-whiteSmoke rounded-3xl max-[1200px]:w-fit max-[1200px]:mx-auto"
                  v-for="user in currentTask.task.performers"
              >
                <NuxtLink
        ...
        <div class="p-6 rounded-3xl bg-white shadow-objectDescription">
          <div class="border rounded-full mx-auto px-3 py-2 w-fit flex items-center gap-3 mb-6">
            <img src="/icons/sroki.svg" alt="sroki">
            <h2 class="dark:text-white">{{ $t('Сроки') }}</h2>
          </div>
          <div class="mt-2 flex flex-col items-center gap-4">
            <div class="flex items-center gap-2">
              ...
              <div class="bg-whiteSmoke p-3 rounded-full">
                {{ new Date(currentTask.task.created_at).toLocaleDateString() }}
              </div>
            </div>
            <div class="flex items-center gap-2">
              <p>
              ...
              <div class="bg-whiteSmoke p-3 rounded-full">
                {{ new Date(currentTask.task.deadline).toLocaleDateString() }}
              </div>
            </div>
          </div>
        </div>
    ...
    <TheModal
        v-if="showDeleteTask"
        @showModal="showDeleteTask = !showDeleteTask"
        resizeClass="!w-4/12 max-[1000px]:!w-11/12"
    >
      <p class="font-bold text-3xl text-center mt-12">
        {{ $t('Вы уверены, что хотите удалить?') }}
      </p>
      <p class="text-redOrange mt-8 text-center">
        {{ $t('Предупреждение!') }}
      </p>
      <p class="text-center mt-2">{{ $t('Удалённые задания не подлежат восстановке') }}</p>
      <div class="flex items-center justify-between mt-6 gap-3">
        ...
        <TheButton
            ...
            @click="currentTask.delete_task({id: currentTask.task.id}, $router); "
        >
          {{ $t('Удалить') }}
        </TheButton>
      </div>
