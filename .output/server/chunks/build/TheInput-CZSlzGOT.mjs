  ...
  setup(__props) {
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (__props.type === "textarea") {
        _push(`<textarea name="" id="" cols="30" rows="10"${ssrIncludeBooleanAttr(__props.required) ? " required" : ""} class="${ssrRenderClass([{ "!bg-red-200 border-red-400 dark:border !outline-none": __props.trimi && __props.required }, "text-lg bg-whiteSmoke dark:text-black resize-none border outline-3 rounded-full focus:outline-golden focus:outline-1 outline-none px-4 py-2 dark:bg-gray-300"])}"${ssrRenderAttr("placeholder", __props.label)}>${ssrInterpolate(__props.modelValue)}</textarea>`);
...
//# sourceMappingURL=TheInput-CZSlzGOT.mjs.map
