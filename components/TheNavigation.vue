<template>
  <div class="container z-50 relative shadow-lg dark:border-none"
       :class="{'blur' : useTreeStore().treeFullScreen === true}">
    <div class="flex w-full items-center transition-all justify-between text-center dark:bg-gray-600"
    ...
    <div
        class="py-2 flex flex-col gap-y-4 transition-all duration-300 dark:bg-gray-600 max-h-screen max-[1150px]:pb-24 overflow-y-auto h-screen "
        :class="{'pl-0' : !loadCurrentUser.activeNav, 'max-md:w-full overflow-x-hidden ' : loadCurrentUser.activeNav, 'top-0 fixed shadow-lg dark:border-none': loadCurrentUser.my_scroll>90, 'max-[1151px]:w-[27%] max-[1149px]:w-full max-lg:w-full' : loadCurrentUser.activeNav && loadCurrentUser.my_scroll>100}">
      <NavigationBlock>
        <template v-slot:navTitleIcon>
          <div class="dark:fill-white fill-black">
            <svg width="45px" height="45px" viewBox="-6 0 32 32" version="1.1" xmlns="http://www.w3.org/2000/svg">
              <path
      ...
      <NavigationBlock
        ...
        <template v-slot:navTitleIcon>
          <div class="dark:fill-white fill-black ml-1.5">
            <svg width="40px" height="40px" viewBox="0 0 256 256" id="Flat" xmlns="http://www.w3.org/2000/svg">
              <path
      ...
      <div class="h-full transition-all duration-500 mx-auto max-md:!ml-3">
        <TheButton
          ...
          <p v-if="loadCurrentUser.activeNav" class="font-medium dark:text-white text-black">{{ $t("Выйти") }}</p>
        </TheButton>
      </div>
    </div>
  </div>
